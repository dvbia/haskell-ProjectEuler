...
bigger _ [] = []
bigger [] _ = []
bigger (a:as) (b:bs) = if a > b 
			then a:(bigger as bs)
			else b:(bigger as bs)
			

accumulate parent row = bigger ( zipWith (+) (0:parent) row ) ( zipWith (+) (parent++[0]) row ) 
			
solve input = maximum $ foldl accumulate (head input) (tail input)
